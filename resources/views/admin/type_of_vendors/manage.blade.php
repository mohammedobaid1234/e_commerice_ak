...
<script>
    $(function () {
        var table = $('#kt_datatable').DataTable({
          ...
          buttons: table_btns,
          ajax: {
            url:  "{{ route('type_of_vendors.manage') }}",
            data: function (d) {
...
<script>
    $(document).on('click','.edit_item_btn',function (e) {
       ...
       var id = $(this).data("id");
        $.ajax({
            url: $("meta[name='BASE_URL']").attr("content") + '/admin/type_of_vendors/' + id + '/edit', 
            type: "get", 
...
<script>
    $(document).on('click','.create_item_btn',function (e) {
       ...
       var id = $(this).data("id");
        $.ajax({
            url: $("meta[name='BASE_URL']").attr("content") + '/admin/type_of_vendors/create', 
            type: "get", 
            success: function (response) {
                if(response.code ==200){
